...
- Shared PostgreSQL lifecycle and provisioning (cluster, managed volume, DB/role provisioning)
- Shared PostgreSQL backup and restore operations

Does not own:
- Domain behavior of business services
- Service schema business rules or application-level migration semantics
...
2. `platform-foundation` owns PostgreSQL infrastructure lifecycle and backup/restore operations.
3. Service runtime roles are least-privilege and scoped to their own database only.
4. Services interact via explicit contracts (OpenAPI/events), not shared tables.
5. UI never bypasses services to hit databases directly.
6. Partner protocols are isolated in adapters; core platform remains protocol-agnostic.
7. Channel policy stays centralized in `channel-policy-router`.
...
5. Shared PostgreSQL operational contracts are defined in `docs/specs/postgresql-ownership-and-operations.md`.
