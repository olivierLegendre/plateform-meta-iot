...
## 1.1 Implementation status reference (normative linkage)

This document defines the V1 target contract. Delivery status is tracked in:

- `docs/project-monitoring/overview/service-readiness-tracker.md`

Interpretation rule:

1. This specification is normative for scope and behavior.
2. The readiness tracker is normative for execution status, blockers, and evidence.
3. If a mismatch appears, update the tracker first for runtime facts, then align this spec if the target contract changed.

Current platform-foundation verification baseline (2026-04-03):

1. Wave 1 observability baseline: PASS.
2. Wave 1 vault runtime baseline: PASS.
3. Wave 6 topology release gate: PASS.
4. Wave 8 namespace readiness: PASS.
5. Python gate scripts resolve runtime as `PYTHON_BIN` override, then repo `.venv/bin/python`, then `python3`.
...
## 2.1 PoC parity requirement (mandatory)

V1 must preserve at least the same functional service level as the validated PoC baseline at
`/home/olivier/Public/poc`.

Interpretation rule:

1. If a PoC capability existed and was used in operations or acceptance, V1 must provide an equivalent capability.
2. Equivalence can be implemented in different components, but user-level and operational outcomes must not regress.
3. Any temporary gap versus PoC must be explicitly documented, risk-rated, and tracked with an owner and target date.

Normative parity reference:
...
5. Operator UI: React (React 18 + React Router + Vite).
...
3. Separate least-privilege DB roles per service (`svc_<service>_app`, `svc_<service>_migrator`).
4. Service roles are not PostgreSQL `SUPERUSER` and do not have global `CREATEDB` or `CREATEROLE`.
...
- `docs/specs/postgresql-ownership-and-operations.md`
